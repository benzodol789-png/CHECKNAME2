...
| `DAY_RESET_HOUR` | `2` | สถิติรายวัน (`/api/daily-stats`) ตัดรอบวันใหม่ตอนกี่โมง เวลาไทย |
| `DAILY_STATS_TTL` | `10` | cache ผลสถิติรายวันกี่วินาที |
...
## สถิติรายวัน (โปรแกรมกระดิ่งทองดึงไปแสดง)

`GET /api/daily-stats` — สรุปรายคนว่าวันนี้ออกไปกิจกรรมไหนกี่ครั้ง รวมกี่วินาที และเกินเวลากี่ครั้ง
ใส่ `?day=YYYY-MM-DD` เพื่อขอย้อนหลังได้ (ไม่ใส่ = วันทำงานปัจจุบัน)

- ตัดรอบวันใหม่ตอน `DAY_RESET_HOUR` น. เวลาไทย (ค่าเริ่มต้นตี 2) ไม่ใช่เที่ยงคืน เพราะกะดึกเลิก 08:00
- "กี่ครั้ง" นับตอนที่เริ่มออกในวันนั้น ส่วน "กี่วินาที" นับเฉพาะช่วงที่อยู่ในวันนั้น
- "เกินเวลา" ตัดสินจากความยาวจริงของรอบนั้นทั้งรอบ เทียบกับโควตาใน `OVERTIME_LIMITS` (กินข้าว 30 / ปวดหนัก 20 / ปวดน้อย 6 นาที)
- รอบที่ยังไม่กดกลับที่นั่ง นับเวลาถึงตอนนี้ และรายงานแยกไว้ที่ `current`

เซิร์ฟเวอร์กระดิ่งทอง (`goldenbell.jed89.com`) ดึงข้อมูลนี้ไปแสดงในโปรแกรมพนักงาน/แอดมิน
และใช้เตือนพนักงานก่อนหมดเวลา 1 นาที — อ่านอย่างเดียว ไม่เขียนอะไรกลับมา
