...
| `SHIFT_DAY_HOUR` | `8` | กะเช้าเริ่มกี่โมง (เวลาไทย) — จุดรีเซ็ตสถิติรอบเช้า |
| `SHIFT_NIGHT_HOUR` | `20` | กะดึกเริ่มกี่โมง (เวลาไทย) — จุดรีเซ็ตสถิติรอบดึก |
| `DAILY_STATS_TTL` | `10` | cache ผลสถิติกี่วินาที |
...
## สถิติรายกะ (โปรแกรมกระดิ่งทองดึงไปแสดง)

`GET /api/daily-stats` — สรุปรายคนว่ากะนี้ออกไปกิจกรรมไหนกี่ครั้ง รวมกี่วินาที และเกินเวลากี่ครั้ง
ใส่ `?day=YYYY-MM-DD&shift=เช้า|ดึก` เพื่อขอย้อนหลังได้ (ไม่ใส่ = กะที่กำลังทำงานอยู่)

- นับเป็นรอบกะ **08:00-20:00** และ **20:00-08:00** ให้ตรงกับที่ระบบรีเซ็ตโควตา 2 รอบต่อวัน
  (ตอบกลับมาพร้อม `shift`, `shift_label`, `day_short` ให้เอาไปแสดงได้เลย)
- "กี่ครั้ง" นับตอนที่เริ่มออกในกะนั้น ส่วน "กี่วินาที" นับเฉพาะช่วงที่อยู่ในกะนั้น
